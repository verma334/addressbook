<?php
 ...
 $get_list_sql = "SELECT id,CONCAT_WS(', ', f_name, l_name) AS display_name FROM master_name ORDER BY l_name, f_name";
...
//get all tel
 $get_tel_sql = "SELECT tele_number, type FROM telephone WHERE master_id = '".$safe_id."'";
 ...
 $display_block .= "<p><strong>Telephone:</strong><br/> <ul>";
  while ($tel_info = mysqli_fetch_array($get_tel_res)) {
     $tele_number = stripslashes($tel_info['tele_number']);
     $tel_type = $tel_info['type'];

     $display_block .= "<li>$tele_number ($tel_type)</li>";
    }
  $display_block .= "</ul>";
 }
 //free result
